...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['turno_id'])) {
    ...
    // Actualizar el estado del turno a 'attended'
    $sql = "UPDATE turnos SET status='attended', called_by='$module' WHERE id=$turno_id";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        $response['message'] = "Turno atendido desde el módulo $module: {$row['service_code']}{$row['turno_numero']}";
    } else {
        $response['success'] = false;
        $response['message'] = "Error al atender el turno: " . $conn->error;
    }
} else {
    $response['success'] = false;
